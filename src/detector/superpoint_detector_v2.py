...
class SuperPoint_Feat(nn.Module):
    ...
    def detect(self, img_path):
        ...
        tensor = torch.from_numpy(gray0/255.).float()[None, None]
        if torch.cuda.is_available():
            tensor = tensor.cuda()

        # Shared Encoder
        x = self.relu(self.conv1a(tensor))
        ...
        scores = simple_nms(scores, self.nms_dist)
        ...
        if self.chosen_num >= 0:
            keypoints, scores = list(zip(*[
                top_k_keypoints(k, s, self.chosen_num)
                for k, s in zip(keypoints, scores)]))
        ...
        return {
            ...
            'numpy':{
                ...
                'descriptors': descriptors[0].cpu().detach().numpy().T
            }
        }
